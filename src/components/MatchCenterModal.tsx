...
const MatchCenterModal: React.FC<MatchCenterModalProps> = ({ match, onClose }) => {
  ...
  const renderContent = () => {
    if (loading) return <div className="flex justify-center items-center h-64"><Loader2 size={32} className="animate-spin text-primary" /></div>;
    if (!details) return <div className="flex flex-col justify-center items-center h-64 text-slate-500"><AlertCircle size={32} className="mb-2" /><p>تفاصيل المباراة غير متاحة حالياً.</p></div>;
        ...
        return (
             <div className="animate-in fade-in">
                <ul className="space-y-3">
                    ...
                    )) : <div className="text-center text-slate-500 py-10">لا توجد أحداث رئيسية مسجلة.</div>}
                </ul>
             </div>
        );
      default: return null;
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative w-full max-w-2xl bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-4 border-b border-slate-800 bg-slate-950">
          <div className="flex items-center gap-2 text-sm text-slate-400"><MapPin size={14}/> {match.league}</div>
          ...
          <div className="flex items-center gap-2 text-sm text-slate-400"><Clock size={14}/> {new Date().toLocaleDateString('ar-SA')}</div>
        </div>
        <div className="p-6 bg-slate-800/50 flex items-center justify-around text-white">
            ...
            <div className="px-6 text-center">
                 <div className="text-4xl font-black font-mono tracking-wider">
                     {match.status === 'UPCOMING' ? <span className="text-slate-500 text-3xl">{match.time}</span> : <span>{match.scoreHome} - {match.scoreAway}</span>}
                 </div>
