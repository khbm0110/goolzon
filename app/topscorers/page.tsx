...
export default async function TopScorersPage() {
  ...
  const scorers = clubs
    ...
    .filter((entry) => (entry.player.seasonStats?.goals ?? 0) > 0)
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-2xl">
      <h1 className="text-3xl font-black text-[var(--fg)] mb-8 flex items-center border-r-4 border-primary pr-4">
        <Target className="ml-3 text-primary" size={32} />
        هدافو الموسم
      </h1>
      ...
      {scorers.length === 0 ? (
        <div className="text-center py-20 bg-[var(--bg-surface)] rounded-xl border border-[var(--border-subtle)] border-dashed text-[var(--fg-faint)]">
          لا توجد إحصائيات أهداف مسجلة حالياً.
        </div>
      ) : (
        <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-2xl overflow-hidden divide-y divide-[var(--border-subtle)]">
          {scorers.map((entry, idx) => (
            <Link
              ...
              className="flex items-center gap-4 p-4 hover:bg-[color-mix(in_srgb,var(--bg-surface-2)_30%,transparent)] transition-colors"
            >
              ...
              <div className="w-10 h-10 rounded-full bg-[var(--bg-surface-2)] overflow-hidden flex-shrink-0 border border-[var(--border)] relative">
                {entry.player.image ? (
              ...
              <span className="text-2xl font-black text-primary">{entry.player.seasonStats?.goals}</span>
            </Link>
          ))}
        </div>
      )}
