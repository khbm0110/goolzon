...
export default function ProfilePage() {
  ...
  const handleExportImage = async () => {
    ...
    try {
      ...
      const dataUrl = await toPng(exportRef.current, { cacheBust: true, quality: 0.95, backgroundColor: '#020617', style: { transform: 'none' } });
      ...
      <div ref={exportRef} className="relative h-[550px] md:h-[600px] bg-[var(--bg-surface)] group border-y mt-6 border-[var(--border-subtle)] overflow-hidden">
        ...
        <div className="absolute inset-0">
          {!showTactics ? (
            ...
            <InteractivePitch squad={localDreamSquad} onSlotClick={handleSlotClick} onRemovePlayer={handleRemovePlayer} isLoading={profileLoading} />
          )}
...
function InteractivePitch({
  ...
  return (
    <div className="w-full h-full bg-emerald-800 relative overflow-hidden flex justify-center items-center shadow-inner">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-24 h-24 md:w-32 md:h-32 border-2 border-white/20 rounded-full pointer-events-none" />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1.5 h-1.5 bg-white/50 rounded-full pointer-events-none" />
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-1/3 h-16 md:h-24 border-2 border-white/20 rounded-b-xl border-t-0 pointer-events-none" />
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-1/3 h-16 md:h-24 border-2 border-white/20 rounded-t-xl border-b-0 pointer-events-none" />
...
function PlayerSelectorModal({
  ...
  const filtered = (lowerQuery === '' ? players : players.filter((p) => p.name.toLowerCase().includes(lowerQuery) || p.clubName?.toLowerCase().includes(lowerQuery))).sort(
